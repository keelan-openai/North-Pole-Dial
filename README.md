...
Festive voice call experience that connects a browser directly to the OpenAI Realtime API over WebRTC. Fill the built-in form with your child's details, press call, and Santa greets her with a warm, custom conversation. The UI keeps the magic on screen while every utterance is saved quietly to a text file for you to review later.
...
- Festive single-page UI with a Santa call screen plus a collapsible “Santa’s notes” form for child details (stored in the browser, not on disk).
...
- `SANTA_VOICE` – voice name for output (defaults to `cedar`).
...
- On serverless hosts like Vercel, transcripts are written to `/tmp/transcripts` by default and will not persist between runs. Point `TRANSCRIPT_DIR` to persistent storage if you need durable logs.
